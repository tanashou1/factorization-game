...
### Added
- **遊び方にビジュアル図解を追加**: 「遊び方」モーダルに操作方法とタイル反応のイラストを追加
  - タイルをスワイプ：ミニボードで移動前後を図示
  - 空きマスをスワイプ：全タイルが一緒に動く様子を図示
  - 空きマスをタップ：新タイルが出現する様子を図示
  - 約数反応（割り算）：「6」と「2」→「3」の図解
  - 同値消滅：「5」と「5」→消滅の図解
...
- **新規タイル生成方法**: タイル生成ロジックを変更
  - maxPrime以下の素数から2～10個の素数を選択（重複を許す）
